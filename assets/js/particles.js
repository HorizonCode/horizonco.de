...
var end_panel = document.querySelector("body");
...
function start_fx() {
    ...
    init_fx(
        ...
        0,end_cvWidth,0,end_cvHeight,   // start position x-+, y-+
...
function init_fx(textureSpr, totalSpr, minWidth,maxWidth, minHeight,maxHeight, xMin,xMax, yMin,yMax, veloMin,veloMax, angleMin,angleMax, startScaleMin,startScaleMax, endScaleMin,endScaleMax, rotStart, rotEndMin,rotEndMax, minDur,maxDur, fadeInDur, fadeOutDur, gravitySpr, delaySpr, repeatSpr, delayTl) {
    ...
    createMagicDust = (x,y,n) => {
        for (var i = 0; i < n; i++) {
            var texture = createShape(textureSpr, Math.floor(Math.random()*20));
    ...
    function createShape(textureSpr, i) {
        ...
        if (i % 3 === 0){
            gradient.addColorStop(0, "rgb(159 245 64 / 75%)");
            gradient.addColorStop(0.15,	"rgb(191 245 185 / 18%)");
        } else if (i % 5 === 0){
            gradient.addColorStop(0, "rgb(21 233 255 / 60%)");
            gradient.addColorStop(0.1,	"rgb(185 245 239 / 18%)");
        } else {
            gradient.addColorStop(0, "rgb(245 206 64 / 75%)");
            gradient.addColorStop(0.1,	"rgb(233 245 185 / 18%)");
        }
